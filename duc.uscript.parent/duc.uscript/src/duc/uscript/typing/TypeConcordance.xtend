...
//	}
	...
	def static boolean isPoissonBinomial(Class c) {
		return is(c, POISSBIN_TYPE)
	}
	...
	def static String getTypeFromUType(Class c) {
		...
		if(fqn == GAUSSIAN_DOUBLE_TYPE || fqn == RAYLEIGH_DOUBLE_TYPE || fqn == DIRAC_DOUBLE_TYPE || fqn == MULTPOSS_DOUBLE_TYPE || fqn == UNCERTAIN_NUMERIC_DOUBLE_TYPE) {
			return DOUBLE_TYPE
		}
		
		if(fqn == GAUSSIAN_FLOAT_TYPE || fqn == RAYLEIGH_FLOAT_TYPE || fqn == DIRAC_FLOAT_TYPE || fqn == MULTPOSS_FLOAT_TYPE || fqn == UNCERTAIN_NUMERIC_FLOAT_TYPE) {
			return FLOAT_TYPE
		}
		
		if(fqn == DIRAC_BYTE_TYPE || fqn == BINOMIAL_BYTE_TYPE  || fqn == MULTPOSS_BYTE_TYPE || fqn == UNCERTAIN_NUMERIC_BYTE_TYPE) {
			return BYTE_TYPE
		}
		
		if(fqn == DIRAC_INT_TYPE || fqn == BINOMIAL_INT_TYPE  || fqn == POISSBIN_INT_TYPE || fqn == MULTPOSS_INT_TYPE || fqn == UNCERTAIN_NUMERIC_INT_TYPE) {
			return INT_TYPE
		}
		
		if(fqn == DIRAC_SHORT_TYPE || fqn == BINOMIAL_SHORT_TYPE || fqn == MULTPOSS_SHORT_TYPE || fqn == UNCERTAIN_NUMERIC_SHORT_TYPE) {
			return SHORT_TYPE
		}
		
		if(fqn == DIRAC_LONG_TYPE || fqn == BINOMIAL_LONG_TYPE || fqn == MULTPOSS_LONG_TYPE || fqn == UNCERTAIN_NUMERIC_LONG_TYPE) {
			return LONG_TYPE
		}
